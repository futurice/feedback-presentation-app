		...
		var worker = new Worker (
				window.URL.createObjectURL(
					new Blob(
						...
						['self.onmessage = function ( wrk ) { importScripts("http://localhost:5000/bower_components/underscore/underscore.js"); var f = '+ Function.toString.call(thread) +'; self.postMessage( { status : "done", result : f.apply(f, wrk.data.args ) } ); '+ ( config.once ? ' self.close(); ' : '' ) +' };']
